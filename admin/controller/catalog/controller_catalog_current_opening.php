<?php
	class Controller_Current_Opening {
		
		public	$model;
		

		public function __construct() {
			require_once('model/catalog/model_catalog_current_opening.php');
			$this->model =  new Model_Current_Opening();
		}
		...
		public function index() {
			...
			include_once('view/templates/catalog/current_opening/template_catalog_current_opening_index.php');
		...
		public function insert() {
			...
			if(isset($_POST[info])) {
				...
				$error = empty($dbInfo[title]) || strlen($dbInfo[title])<2 || strlen($dbInfo[title])>100 ? "Field value must be between 2 and 100 characters!" : "" ;
				if(!empty($error))
					...
					if($this->model->insert($dbInfo)) {
						echo '<script>window.location.href="index.php?route=catalog/current_opening"</script>';
						exit();
					} else {
			...
			global $header; global $menu; global $footer;
			include_once('view/templates/catalog/current_opening/template_catalog_current_opening_form.php');
