<?php
	...
	class Model_Faq extends Emkt_Model {
			...
			public function getNameById($id) {
				return $this->fetch_singleField("question", "id='$id'");	
			}
			
			public function getIdByName($name) {
				return $this->fetch_singleField("id", "question='$name'");	
			}
			...
			public function getDetailsByName($name) {
				return $this->fetch_single("*","question='$val'");
			}

			public function getDetailsByFilter($filter) {
				return $this->fetch_all("id,question", "question like '$filter%'");
			}
			...
			public function getNewCount() {
				$returnData = $this->fetch_single("Count(*) as Total", "deleted=0 AND status=0");
